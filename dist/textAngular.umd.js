...
.factory('taToolFunctions', ['$window','taTranslations', function($window, taTranslations) {
    return {
        ...
        aOnSelectAction: function(event, $element, editorScope){
            ...
            var targetToggle = angular.element('<button type="button" class="btn btn-default btn-sm btn-small toggle" tabindex="-1" unselectable="on">' + taTranslations.editLink.targetToggle.buttontext + '</button>');
            ...
            /* george adding new no-follow button*/
            var followToggle = angular.element('<button type="button" class="btn btn-default btn-sm btn-small toggle" tabindex="-1" unselectable="on">' + taTranslations.editLink.noFollow.buttontext + '</button>');
            if($element.attr('rel') === 'nofollow'){
                followToggle.addClass('active');
            }
            followToggle.on('click', function(event){
                event.preventDefault();
                $element.attr('rel', ($element.attr('rel') === 'nofollow') ? '' : 'nofollow');
                followToggle.toggleClass('active');
                editorScope.updateTaBindtaTextElement();
            });
            buttonGroup.append(followToggle);
...
var textAngularVersion = 'v1.5';   // This is automatically updated during the build process to the current release!
                    ...
                    var processpaste = function(text) {
                       ...
                       var _isOneNote = text!==undefined? text.match(/content=["']*OneNote.File/i): false;
                        ...
                        if(text && text.trim().length){
                            ...
                            // GEORGE EDIT removing crap from pasted content - brace for impact
                            text = text.replace( /<\/?span[^>]*>/g, "" ); // remove all spans from pasted html in HTML VIEW
                            text = text.replace( /style="[^"]*"/g,  "" ); // remove all style attributes in HTML VIEW
                            text = text.replace( /class="[^"]*"/g,  "" ); // remove all classes in HTML VIEW
                            text = text.replace( /<h1><\/h1>/g,  "" ); // remove all empty h1 IN HTML VIEW
                            text = text.replace( /<h2><\/h2>/g,  "" ); // remove all empty h2 IN HTML VIEW
                            text = text.replace( /<h3><\/h3>/g,  "" ); // remove all empty h3 IN HTML VIEW
                            text = text.replace( /<h4><\/h4>/g,  "" ); // remove all empty h4 IN HTML VIEW
                            text = text.replace( /<h5><\/h5>/g,  "" ); // remove all empty h5 IN HTML VIEW
                            text = text.replace( /<h6><\/h6>/g,  "" ); // remove all empty h6 IN HTML VIEW
                            text = text.replace( /<div><\/div>/g,  "" ); // remove all empty div IN HTML VIEW
                            text = text.replace( /<p><\/p>/g,  "" ); // remove all <p></p> IN HTML VIEW
                            text = text.replace( /<p>\s*<\/p>/g,  "" ); // remove all <p></p> IN HTML VIEW
                            text = text.replace( /<!--[\s\S]*?-->/g,  "" ); // remove all html comments IN HTML VIEW
                            text = text.replace( /<p><br>\s*<\/p>/g,  "" ); // remove all <p><br></p> IN HTML VIEW
                            text = text.replace( /<p>\s*<br>\s*<\/p>/g,  "" ); // remove all <p><br></p> IN HTML VIEW
                            text = text.replace( /<p>\s*<br><br>\s*<\/p>/g,  "" ); // remove all <p><br></p> IN HTML VIEW

                            while ( text.indexOf ( "<p></p>" ) === 0 || text.indexOf ( "<p><br/></p>" ) === 0 || text.indexOf ( "<p><br></p>" ) === 0 ) {
                                if ( text.indexOf ( "<p></p>" ) === 0  ) text.replace ( "<p></p>", "" );
                                if ( text.indexOf ( "<p><br/></p>" ) === 0  ) text.replace ( "<p><br/></p>", "" );
                                if ( text.indexOf ( "<p><br></p>" ) === 0  ) text.replace ( "<p><br></p>", "" );
                            }
